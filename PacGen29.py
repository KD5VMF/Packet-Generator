...
class PacketGeneratorApp:
    ...
    def create_widgets(self):
        ...
        ttk.Entry(target_frame, textvariable=self.target_ip).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ...
        ttk.Entry(target_frame, textvariable=self.target_port).grid(row=0, column=3, padx=5, pady=5, sticky="ew")
        ...
        ttk.Entry(packet_frame, textvariable=self.packet_size).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ...
        ttk.Entry(packet_frame, textvariable=self.packets_per_second).grid(row=0, column=3, padx=5, pady=5, sticky="ew")
        ...
        ttk.Entry(packet_frame, textvariable=self.max_packets).grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        ...
        protocol_version_chk = ttk.Checkbutton(features_frame, text="Add Protocol Version",
                                               variable=self.var_protocol_version)
        protocol_version_chk.grid(row=1, column=0, sticky="w", padx=5, pady=5)

        source_mac_chk = ttk.Checkbutton(features_frame, text="Add Source MAC Address",
                                         variable=self.var_source_mac)
        source_mac_chk.grid(row=1, column=1, sticky="w", padx=5, pady=5)

        unique_id_chk = ttk.Checkbutton(features_frame, text="Add Unique Identifier",
                                        variable=self.var_unique_id)
        unique_id_chk.grid(row=1, column=2, sticky="w", padx=5, pady=5)

        payload_signature_chk = ttk.Checkbutton(features_frame, text="Add Payload Signature",
                                                variable=self.var_payload_signature)
        payload_signature_chk.grid(row=1, column=3, sticky="w", padx=5, pady=5)
        ...
        autotune_chk.grid(row=2, column=0, sticky="w", padx=5, pady=5)
    ...
    def generate_packet(self, seq_number):
        """
        Build the packet:
           [4-byte seq][(user_size - overhead) 'X' bytes + optional data] + [4-byte CRC32]

        Overhead includes:
           - 4 bytes for sequence number
           - Variable bytes for optional fields
           - 4 bytes for CRC32
        """
        ...
        if self.var_unique_id.get():
            ...
            body += f"UUID:{unique_id.decode('utf-8')};".encode("utf-8")

        # 4) Payload Signature
        if self.var_payload_signature.get():
            # For simplicity, we'll create a SHA256 hash of the current body
            # Note: In a real scenario, ensure thread safety if modifying body before hashing
    ...
    def generate_packet(self, seq_number):
        ...
        Optional fields are appended based on selected checkboxes:
           - Protocol Version
           - Source MAC Address
           - Unique Identifier (UUID)
           - Payload Signature (CRC32 of the payload)
        """
        user_size = self.packet_size.get()
        if user_size < 0:
            user_size = 0
        ...
        if self.var_payload_signature.get():
            ...
            # In real scenarios, consider using more secure hashes like SHA256
            checksum = zlib.crc32(body) & 0xFFFFFFFF
            payload_signature = f"Payload-CRC32:{checksum};".encode("utf-8")
            body += payload_signature
