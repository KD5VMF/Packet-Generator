...
# Define maximum preview length for the packet body (in characters)
...
class PacketGeneratorApp:
    ...
    A Packet Generator with enterprise-level features:
      ...
      - Link LEDs (green if link is up, red if down) and a resizable GUI window
    ...
    def __init__(self, root):
        ...
        # Set up a global style using ttk.Style to use a larger, robotic font.
        ...
        # Bytes sent and received (for speed measurement)
        ...
        self.max_graph_points = 300  # Maximum number of points to keep in memory
        ...
        # Flag for non-verbose mode (to log a one-time starting message)
        self.sent_start_logged = False
        ...
        self.update_lost_packets_label()
    ...
    def update_lost_packets_label(self):
        # Update lost packets label continuously.
        self.lost_packet_label.config(text=str(self.lost_packets_total),
                                      fg="red" if self.lost_packets_total > 0 else "black")
        self.root.after(500, self.update_lost_packets_label)
    ...
    def create_widgets(self):
        ...
        ttk.Label(title_frame, text="Packet Generator", style="Title.TLabel").pack(fill='x', padx=10)
        ...
        ttk.Label(net_config_frame, text="Select Source Port:", anchor="w").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.source_port = tk.StringVar()
        self.source_port_combo = ttk.Combobox(net_config_frame, textvariable=self.source_port, state="readonly", width=50)
        self.source_port_combo.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ttk.Label(net_config_frame, text="Select Destination Port:", anchor="w").grid(row=0, column=2, sticky="w", padx=5, pady=5)
        self.destination_port = tk.StringVar()
        self.destination_port.trace("w", self.auto_set_target_ip)
        self.destination_port_combo = ttk.Combobox(net_config_frame, textvariable=self.destination_port, state="readonly", width=50)
        self.destination_port_combo.grid(row=0, column=3, padx=5, pady=5, sticky="ew")
        ttk.Label(net_config_frame, text="Source Link:", anchor="w").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(net_config_frame, text="Destination Link:", anchor="w").grid(row=1, column=2, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(target_frame, text="Target IP:", anchor="w").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(target_frame, text="Target Port:", anchor="w").grid(row=0, column=2, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(packet_frame, text="Packet Size (Bytes):", anchor="w").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(packet_frame, text="Packets per Second:", anchor="w").grid(row=0, column=2, sticky="w", padx=5, pady=5)
        ...
        ttk.Label(packet_frame, text="Max Packets (0=Infinite):", anchor="w").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ...
        # Features and Preconfig Settings
        ...
        # Use a tk.Label here so we can update the fg color.
        self.lost_packet_label = tk.Label(counters_frame, text="0", font=("Roboto", 12), fg="black")
    ...
    def update_leds(self, canvas, status):
        # LED is green if up, red if down.
        canvas.config(bg="green" if status else "red")
    ...
    def clear_all(self):
        # Clear the debug log completely without logging a clear message.
        self.debug_text.config(state="normal")
        ...
        self.lost_packet_label.config(text="0", fg="black")
        ...
        while self.running:
            ...
            if current_time >= next_send_time:
                try:
                    ...
                    # Extract the packet body (exclude the 4-byte sequence header and 4-byte CRC)
                    body_bytes = packet[4:-4]
                    try:
                        packet_preview_ascii = body_bytes.decode('ascii', errors='replace')
                    except Exception as e:
                        packet_preview_ascii = "<non-decodable>"
                    packet_preview_ascii = re.sub(r'X{20,}', '[FILLER]', packet_preview_ascii)
                    if len(packet_preview_ascii) > MAX_PREVIEW_LENGTH:
                        packet_preview_ascii = packet_preview_ascii[:MAX_PREVIEW_LENGTH] + "..."
                    if self.var_verbose.get():
                        if current_count % 100 == 0:
                            self.log(f"Sent packet #{current_count} ({bytes_sent} bytes). Preview: {packet_preview_ascii}")
                    else:
                        if not self.sent_start_logged:
                            self.log(f"Started sending packets in non-verbose mode. (Packet #{current_count} sent)")
                            self.sent_start_logged = True
