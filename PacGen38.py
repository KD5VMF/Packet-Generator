...
class PacketGeneratorApp:
    ...
    def __init__(self, root):
        ...
        # This flag is used to log a one-time starting message when verbose is off.
            ...
            if current_time >= next_send_time:
                try:
                    ...
                    if self.var_verbose.get():
                        # In verbose mode, log detailed preview every 100 packets
                        if current_count % 100 == 0:
                            self.log(f"Sent packet #{current_count} ({bytes_sent} bytes). Preview: {packet_preview_ascii}")
                    else:
                        # In non-verbose mode, log one starting message only
