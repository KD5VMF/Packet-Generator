...
class PacketGeneratorApp:
    ...
    Features include optional handshake, latency measurement, HMAC signing, random payload generation,
    extended header options, and real-time header overhead calculation.
    Additionally, once a network interface is selected for a port, it is removed from the list for other ports,
    until the user presses Clear, which repopulates all available interfaces.
    """
    def __init__(self, root):
        ...
        # Define target_port before dependent methods.
        self.target_port = tk.IntVar(value=12345)

        # Configure main window: top for controls/status, bottom for debug log.
        ...
        self.port_comboboxes = {}  # We'll store the combobox widgets here.
        self.port_vars = {}        # And each port's StringVar.
        self.port_tx_vars = {}
        self.port_rx_vars = {}
        ...
        self.var_ext_a = tk.BooleanVar(value=False)  # adds 50 bytes
        self.var_ext_b = tk.BooleanVar(value=False)  # adds 100 bytes

        # To store the full available interfaces list.
    ...
    def compute_header_overhead(self):
        ...
        if self.var_hmac.get():
            overhead += 70  # "HMAC:" + 64 hex digits + ";" = 70 bytes
    ...
    def update_header_length_label(self):
        ...
        max_filler = MAX_UDP_PAYLOAD - (overhead + 4)  # 4 bytes for final CRC
    ...
    def create_controls(self, parent):
        ...
        for i, port in enumerate(PORT_NAMES):
            ...
            var = tk.StringVar()
            # Add trace to update interface lists when selection changes
            var.trace_add("write", self.update_interface_lists)
    ...
    def update_interface_lists(self, *args):
        # Update the combobox lists to exclude interfaces already selected on other ports.
        selected = set()
        for port in PORT_NAMES:
            val = self.port_vars.get(port)
            if val:
                current = self.port_comboboxes[port].get()
                if current:
                    selected.add(current)
        for port in PORT_NAMES:
            current_selection = self.port_comboboxes[port].get()
            allowed = [iface for iface in self.available_interfaces if iface not in (selected - {current_selection})]
            self.port_comboboxes[port]['values'] = allowed
            if current_selection not in allowed and allowed:
                self.port_comboboxes[port].set(allowed[0])
                ...
                for addr in iface_info:
                    if addr.family == socket.AF_INET:
                        if addr.address.startswith("127.") or addr.address.startswith("169.254."):
                            continue
                        self.available_interfaces.append(f"{iface_name} - {addr.address}")
            for port in PORT_NAMES:
                self.port_comboboxes[port]['values'] = self.available_interfaces
                if self.available_interfaces:
                    self.port_comboboxes[port].set(self.available_interfaces[0])
            self.log("Detected interfaces: " + str(self.available_interfaces))
        except Exception as e:
    ...
    def generate_packet(self, seq_number):
        ...
        # Extended options
        if self.var_ext_a.get():
            temp_body += b"EXT_A:50;"
        if self.var_ext_b.get():
            temp_body += b"EXT_B:100;"
        max_body_length = final_size - 8  # 4 bytes seq + 4 bytes CRC
    ...
    def update_interface_lists(self, *args):
        # Get the set of interfaces already selected
        selected = set()
        for port in PORT_NAMES:
            val = self.port_comboboxes[port].get()
            if val:
                selected.add(val)
        # For each port, allow all available interfaces except those selected by other ports.
        for port in PORT_NAMES:
            current_selection = self.port_comboboxes[port].get()
            allowed = [iface for iface in self.available_interfaces if iface not in (selected - {current_selection})]
            self.port_comboboxes[port]['values'] = allowed
    ...
    def clear_all(self):
        ...
        self.start_time = time.perf_counter()
        self.random_payload = None
        # Re-detect interfaces to repopulate lists.
        self.detect_network_interfaces()
