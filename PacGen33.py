...
class PacketGeneratorApp:
    ...
    A Packet Generator with enterprise-level features:
      ...
      - Live graph displaying packet sending speed
    """
    ...
    def __init__(self, root):
        ...
        # After callback IDs for cancellation on exit
    ...
    def on_close(self):
        """On close: cancel scheduled callbacks, stop threads, quit and force exit."""
        ...
        try:
            ...
            self.log(f"Error cancelling after callbacks: {e}")
        ...
        # Optionally wait for threads to exit with a timeout.
        ...
        # Quit the mainloop and force exit.
        self.root.quit()
        self.root.destroy()
        os._exit(0)
    ...
    def create_widgets(self):
        """Build all GUI widgets with improved layout."""
        # Configure root for 2 columns: left (configuration) and right (status/controls)
        self.root.grid_columnconfigure(0, weight=1, uniform="group1")
        self.root.grid_columnconfigure(1, weight=1, uniform="group1")
        self.root.grid_rowconfigure(6, weight=1)  # Debug log row expands

        # Title Frame (spans both columns)
        ...
        # Left Column Frames (Configuration)
        ...
        # Right Column Frames (Status & Controls)
        # Counters Frame
        ...
        ttk.Label(speed_frame, text="Speed:", anchor="w")\
            .grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        # Live Graphics Frame (placed under the Controls Frame)
        self.graph_frame = ttk.LabelFrame(self.root, text="Live Graph")
        self.graph_frame.grid(row=4, column=1, padx=10, pady=5, sticky="nsew")
        self.root.grid_rowconfigure(4, weight=1)  # Let the graph frame expand vertically
        ...
        # Set up the line object for updating the plot
        self.graph_line, = self.ax.plot([], [], lw=2)
        self.ax.set_title("Packet Sending Speed (Mbps)")
        self.ax.set_xlabel("Time (s)")
        self.ax.set_ylabel("Speed (Mbps)")

        # Status Frame (spans both columns)
        ...
        # Debug Log Area Frame (spans both columns)
        ...
        # Start the live graph update loop
    ...
    def update_speed(self):
        ...
        # Append current speed to the live graph data
    ...
    def update_live_graph(self):
        ...
        self.canvas.draw()
        self.after_live_graph_id = self.root.after(1000, self.update_live_graph)
